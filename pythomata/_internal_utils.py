...
def _check_transition_function_is_valid_wrt_states_and_alphabet(transition_function: TransitionFunction,
    ...
    start_states, symbols = list(map(set, zip(*map(lambda x: (x[0], x[1]), transition_function.keys()))))
    end_states = set(transition_function.values())
    ...
    if not all(s in alphabet for s in symbols):
        raise ValueError("Transition function not valid: symbols {} are not in the alphabet."
                         .format(symbols.difference(alphabet)))
